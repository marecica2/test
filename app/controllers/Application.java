...
public class Application extends BaseController {
	...
	public static void contactUs(String uuid, String name, String email, String subject, String message,
			String captcha, String id) {
		final User user = getLoggedUser();
		...
		if (!validation.hasErrors()) {
			...
			if (recipient != null && recipient.emailNotification) {
				...
				eb.setTo(recipient);
				eb.setFrom(user).setSubject(subject).setMessageWiki(body).send();
			}
	...
	public static void userProfile(String userLogin, String id) {
		...
		final String name = user != null ? user.getFullName() : Messages.get("anonymous")
				+ RandomUtil.getRandomDigits(5);
		final String room = null;
		final String rmtp = getProperty(CONFIG_RMTP_PATH);
		final String socketIo = getProperty(CONFIG_SOCKET_IO);

		render(user, usr, userProfile, isOwner, listings, followees, followers, follow, ratings, stats, contact, name,
				room, rmtp, socketIo);
	}
	...
	public static void calendarUser(String login, String channel) throws Throwable {
		...
		renderTemplate("/Application/calendar.html", user, userDisplayed, isOwner, listing, listings, followers,
				followees, follow);
	}
