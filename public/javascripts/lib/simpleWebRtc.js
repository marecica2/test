(function(e) {
    if ("function" == typeof bootstrap)
        ...
        "undefined" != typeof window ? window.SimpleWebRTC = e()
                : global.SimpleWebRTC = e()
})
        (function() {
...
SimpleWebRTC.prototype.handlePeerStreamAdded = function (peer) {
    ...
    // xxx my event fired when video was added
    this.connection.emit('videoAdded', peer.id);
};
...
SimpleWebRTC.prototype.handlePeerStreamRemoved = function (peer) {
    ...
    // xxx my event fired when video was removed
    this.connection.emit('videoAdded', peer.id);
};
    ...
    this.connection.emit('join', name, function (err, roomDescription) {
        if (err) {
            ...
            for (id in roomDescription.clients) {
                ...
                //if peer exists dont create new one
                var peers = self.webrtc.peers;
                var alreadyExists = false;
                for(var j = 0; j < peers.length; j++){
                    if(peers[j].id == id){
                        console.log("$$$$$$$$$$$ peer already exists " + id);
                        alreadyExists = true;
                    }
                }
                
                for (type in client) {
                    if (client[type] && !alreadyExists) {
                        peer = self.webrtc.createPeer({
                            id: id,
                            type: type
                        });
